...
//
// $Id: mainWindow.cpp,v 1.22.2.2 2009/07/23 21:18:12 cmatei Exp $
//
//######################################################################
...
QObject::connect(mUI->helpPolycrankAction, SIGNAL(triggered()), this, SLOT(helpPolycrank()));
	...
	// -- polycrank menu
	QObject::connect(mUI->PolycrankPanelAction, SIGNAL(triggered()), this, SLOT(PolycrankPanel()));
	QObject::connect(mUI->PolycrankSaverAction, SIGNAL(triggered()), this, SLOT(PolycrankSaver()));
	QObject::connect(mUI->PolycrankPlayerAction, SIGNAL(triggered()), this, SLOT(PolycrankPlayer()));
...
void MainWindow::fileImportRobot()
{
	...
	if ( !fn.isEmpty() ) {
		...
		qDebug() << "world file Import Robot = " << world ;
	}
}
...
void MainWindow::helpPolycrank()
{
	Ui::PolycrankDlg dlgUI;
	QDialog dlgImpl(mWindow);
	dlgUI.setupUi(&dlgImpl);
	
	QString versionText("Website : ");
    dlgUI.versionLabel->setText( versionText + "www.polycrank.neth.pl");

	QString emailText("Email : ");
    dlgUI.emailLabel->setText( emailText + "mzbikows@gmail.com");
...
//----------------------------------Polycrank menu--------------------------------------------
void MainWindow::PolycrankPanel()
{	
	//getNumRobots=0 - Any robots
	//getNumRobots=1 - Polycrank.       File->Import Robot...->Polycrank.xml
	//getNumRobots=2 - Polycrank & Barrett Hand.File->Import Robot...->PolycrankBarrett.xml	

	std::cout<<"MainWindow::PolycrankPanel() world->getNumRobots() = "<<world->getNumRobots()<<"\n";

	if ( world->getNumRobots() == 0 ) // Only when user doesn't load ANY ROBOTS program catch the exception
	{
		QMessageBox::warning(mWindow,"Error","At first You should load Polycrank by (File->Open) or (File->Import Robot)", QMessageBox::Ok,Qt::NoButton,Qt::NoButton);
		return;
	}
	else if ( world->getNumRobots() == 1 )
	{
		std::cout<<"MainWindow::PolycrankPanel() = "<<world->robotVec[0]->getName().toStdString()<<"\n";

		if(world->robotVec[0]->getName() == "Polycrank")
		{
			PolycrankPanelDlg *dialog = new PolycrankPanelDlg(mWindow, world);
			dialog->show();
		}
		else
		{
			QMessageBox::warning(mWindow,"Error","Only one Polycrank Window can be active !!! ", QMessageBox::Ok,Qt::NoButton,Qt::NoButton);
			return;
		}
	}
	else if ( world->getNumRobots() == 2 )
	{

		std::cout<<"MainWindow::PolycrankSaver() = "<<world->robotVec[0]->getName().toStdString()<<"\n";

		if(world->robotVec[0]->getName() == "Polycrank")
		{
			//deselect dragger of barret hand
			Body *bodyhand = (Body *) world->getCurrentHand();
			graspItGUI->getIVmgr()->deselectBody(bodyhand);
...
void MainWindow::PolycrankSaver()
{	
	//getNumRobots=0 - Any robots
	//getNumRobots=1 - Polycrank.       File->Import Robot...->Polycrank.xml
	//getNumRobots=2 - Polycrank & Barrett Hand.File->Import Robot...->PolycrankBarrett.xml

	std::cout<<"MainWindow::PolycrankSaver() world->getNumRobots() = "<<world->getNumRobots()<<"\n";
	...
	else if ( world->getNumRobots() == 1 || world->getNumRobots() == 2 )
	{
		std::cout<<"MainWindow::PolycrankSaver() = "<<world->robotVec[0]->getName().toStdString()<<"\n";

		if(world->robotVec[0]->getName() == "Polycrank")
		{
			PolycrankSaverDlg *dialog = new PolycrankSaverDlg(mWindow, world);
			dialog->show();
		}
		else
		{
...
void MainWindow::PolycrankPlayer()
{	
	//getNumRobots=0 - Any robots
	//getNumRobots=1 - Polycrank.       File->Import Robot...->Polycrank.xml
	//getNumRobots=2 - Polycrank & Barrett Hand.File->Import Robot...->PolycrankBarrett.xml

	std::cout<<"MainWindow::PolycrankPlayer() world->getNumRobots() = "<<world->getNumRobots()<<"\n";
...
/*
	...
	{
		std::cout<<"MainWindow::PolycrankPlayer() = "<<world->robotVec[0]->getName().toStdString()<<"\n";

		if(world->robotVec[0]->getName() == "Polycrank")
		{
			PolycrankPlayerDlg *dialog = new PolycrankPlayerDlg(mWindow, world);
			dialog->show();
		}
		else
		{
