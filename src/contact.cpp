...
//
// $Id: contact.cpp,v 1.54.2.2 2009/07/23 21:17:58 cmatei Exp $
//
//######################################################################
...
Contact::frictionForceBlockMatrix(const std::list<Contact*> &contacts)
{
	...
	Matrix Rf(Matrix::BLOCKDIAG(&blocks));
	while (!blocks.empty()) {
		delete blocks.back();
		blocks.pop_back();
	}
...
Contact::frictionConstraintsBlockMatrix(const std::list<Contact*> &contacts)
{
	...
	Matrix blockF(Matrix::BLOCKDIAG(&blocks));
	while (!blocks.empty()) {
		delete blocks.back();
		blocks.pop_back();
	}
...
Contact::normalForceSumMatrix(const std::list<Contact*> &contacts)
{
	...
	std::list<Contact*>::const_iterator it;
	for (it=contacts.begin(); it!=contacts.end(); it++) {
		blocks.push_back( new Matrix(Matrix::ZEROES(1, (*it)->numFrictionEdges+1)) );
		blocks.back()->elem(0,0) = 1.0;
	}
	Matrix blockF(Matrix::BLOCKROW(&blocks));
	while (!blocks.empty()) {
		delete blocks.back();
		blocks.pop_back();
	}
...
Contact::localToWorldWrenchBlockMatrix(const std::list<Contact*> &contacts)
{
	...
	Matrix Ro(Matrix::BLOCKDIAG(&blocks));
	while (!blocks.empty()) {
		delete blocks.back();
		blocks.pop_back();
	}
