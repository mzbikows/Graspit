...
//
// $Id: sql_database_manager.cpp,v 1.17.2.1 2009/07/23 21:18:06 cmatei Exp $
//
//######################################################################
...
bool SqlDatabaseManager::SaveGrasps(const vector<Grasp*> graspList) const {
	...
	for(size_t i = 0; i < graspList.size(); ++i){
		...
		//energy
		query_text << -30.0 * graspList[i]->EpsilonQuality() -100.0 * graspList[i]->VolumeQuality() << ", ";
		...
		if (!database_.Query(query_text.str(), &results))
			return false;
	}
	return true;
}
